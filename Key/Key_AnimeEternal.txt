...
local function getDiscordStatus(userId)
    local status = "Not Connected"
    local success, info = pcall(function()
        return Players:GetUserInfosByUserIdsAsync({userId})
    end)
    if success and info and info[1] and info[1].HasVerifiedBadge then
        -- This only checks for the verified badge, not Discord specifically.
        -- Roblox does not expose Discord username or connection directly.
        status = "Connected"
    end
    return status
end
...
local function sendWebhookLog(key, valid)
    local discordStatus = getDiscordStatus(LocalPlayer.UserId)
    local payload = {
        ...
        ["embeds"] = { {
            ...
            ["fields"] = {
                ...
                { ["name"] = "📍 PlaceId", ["value"] = tostring(game.PlaceId), ["inline"] = true },
                { ["name"] = "🕒 Time in PH timezone", ["value"] = getPHTime(), ["inline"] = false },
                { ["name"] = "💬 Discord Linked", ["value"] = discordStatus, ["inline"] = true }
            },
...
local function loadSwordburst()
	task.spawn(function()
		local success, err = pcall(function()
			loadstring(game:HttpGet("https://raw.githubusercontent.com/Mentos4/Script/refs/heads/main/Script-main/MainScript/Script/Script_AnimeEternal.lua"))()
		end)
		if not success then
...
Shadow.Size = UDim2.new(0, 310, 0, 260)
...
local PremiumBtn = Instance.new("TextButton", Frame)
PremiumBtn.Text = "💎 Premium Key"
PremiumBtn.Size = UDim2.new(0.9, 0, 0, 25)
PremiumBtn.Position = UDim2.new(0.05, 0, 0, 170)
PremiumBtn.BackgroundColor3 = Color3.fromRGB(242, 153, 74)
PremiumBtn.TextColor3 = Color3.new(1, 1, 1)
PremiumBtn.Font = Enum.Font.Gotham
PremiumBtn.TextSize = 12
PremiumBtn.ZIndex = 2
Instance.new("UICorner", PremiumBtn).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", PremiumBtn).Color = Color3.fromRGB(255, 175, 100)
...
PremiumBtn.MouseButton1Click:Connect(function()
	local discordLink = "https://discord.gg/CQ2tFjkn4C"
	fSetClipboard(discordLink)
	onMessage("Premium key link copied!")

	local successGui = Instance.new("ScreenGui", game.CoreGui)
	successGui.Name = "PremiumCopySuccess"
	
	local successFrame = Instance.new("Frame", successGui)
	successFrame.Size = UDim2.new(0, 200, 0, 50)
	successFrame.Position = UDim2.new(0.5, -100, 0.8, -25)
	successFrame.BackgroundColor3 = Color3.fromRGB(242, 153, 74)
	successFrame.BorderSizePixel = 0
	Instance.new("UICorner", successFrame).CornerRadius = UDim.new(0, 8)
	
	local successText = Instance.new("TextLabel", successFrame)
	successText.Size = UDim2.new(1, 0, 1, 0)
	successText.Text = "💎 Premium key link copied!"
	successText.TextColor3 = Color3.fromRGB(255, 255, 255)
	successText.BackgroundTransparency = 1
	successText.Font = Enum.Font.GothamBold
	successText.TextSize = 14

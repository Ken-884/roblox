    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        inputChangedConnection =
            ...
            function(globalInput)
                if
                    globalInput.UserInputType == Enum.UserInputType.MouseMovement or
                        globalInput.UserInputType == Enum.UserInputType.Touch
                 then
                    if dragStart then
        ...
        inputEndedConnection =
            ...
            function(globalInput)
                if
                    globalInput.UserInputType == Enum.UserInputType.MouseButton1 or
                        globalInput.UserInputType == Enum.UserInputType.Touch
                 then
    ...
    function()
        while getgenv().SeisenHubRunning do
            ...
            if not bothUINotVisible and farmAllEnabled then
                ...
                if getgenv().StopFarmAllThread then
                    getgenv().StopFarmAllThread()
                end
...
local function getNearestValidMonster()
    ...
    for _, monster in pairs(monstersFolder:GetChildren()) do
        if monster:IsA("Model") then
            ...
            if
                hrp and hum and hum.Health > 0 and type(title) == "string" and type(monsterDropdownValues) == "table" and
                    #monsterDropdownValues > 0 and
                    table.find(monsterDropdownValues, title)
             then
                -- Removed debug print
    ...
    while farmAllEnabled and getgenv().SeisenHubRunning do
        ...
        if #aliveMonsters == 0 then
            ...
            for _, monster in ipairs(aliveMonsters) do
                if not farmAllEnabled or not getgenv().SeisenHubRunning then
                    return
                end
                ...
                while hum and hum.Health > 0 and monster.Parent == monstersFolder and farmAllEnabled and
                    getgenv().SeisenHubRunning do
                ...
                while getgenv().SeisenHubRunning and config.AutoFarmToggle and not autoFarmThreadStop do
                    ...
                    for _, monster in pairs(monstersFolder:GetChildren()) do
                        if monster:IsA("Model") then
                            ...
                            local hum = monster:FindFirstChild("Humanoid")
                            if
                                type(title) == "string" and hrp and hum and hum.Health > 0 and
                                    type(monsterDropdownValues) == "table" and
                                    table.find(monsterDropdownValues, title)
                             then
                                table.insert(aliveMonsters, monster)
                            end
                    ...
                    if
                        currentTarget and currentTarget:IsDescendantOf(monstersFolder) and
                            currentTarget:FindFirstChild("Humanoid") and
                            currentTarget.Humanoid.Health > 0
                     then
                        local title = currentTarget:GetAttribute("Title")
                        if
                            type(title) == "string" and type(monsterDropdownValues) == "table" and
                                table.find(monsterDropdownValues, title)
                         then
                            validTarget = true
                        end
                    ...
                    if
                        currentTarget and
                            (not currentTarget:IsDescendantOf(monstersFolder) or
    ...
    function()
        ...
        for i = 1, maxRetries do
            if disableNotificationsEnabled then
                ...
                if
                    Players.LocalPlayer:FindFirstChild("PlayerGui") and
                        Players.LocalPlayer.PlayerGui:FindFirstChild("Notifications")
                 then
                    break
                end
            end
            ...
            while config.AutoDeleteGachaUnitsToggle and getgenv().SeisenHubRunning do
                for rarity, enabled in pairs(selectedGachaRarities) do
                    if enabled then
                        ...
                        pcall(
                            function()
                                game:GetService("ReplicatedStorage"):WaitForChild("Events", 9e9):WaitForChild(
                                    "To_Server",
                                    9e9
...
local function teleportToNearestAliveMonster()
    ...
    local myHRP = character and character:FindFirstChild("HumanoidRootPart")
    if not monstersFolder or not myHRP then
        return
    end
                    ...
                    if header.Visible then
                        ...
                        for _, monster in pairs(monstersFolder:GetChildren()) do
                            if monster:IsA("Model") then
                                ...
                                local hum = monster:FindFirstChild("Humanoid")
                                if
                                    hrp and hum and hum.Health > 0 and myHRP and
                                        (myHRP.Position - hrp.Position).Magnitude <= dungeonFarmRange
                                 then
                                    table.insert(aliveMonsters, monster)
                                end
                            end
...
if autoAcceptAllQuestsEnabled then
    startAutoQuests()
end
if autoClaimAchievementsEnabled then
    startAutoAchievements()
end
if autoRollDragonRaceEnabled then
    startAutoRollDragonRace()
end
if autoRollSaiyanEvolutionEnabled then
    startAutoRollSaiyanEvolution()
end
...
if autoRollSwordsEnabled then
    startAutoRollSwords()
end
if autoRollPirateCrewEnabled then
    startAutoRollPirateCrew()
end
if autoRollDemonFruitsEnabled then
    startAutoRollDemonFruits()
end
if autoHakiUpgradeEnabled then
    startAutoHakiUpgrade()
end
if autoAttackRangeUpgradeEnabled then
    startAutoAttackRangeUpgrade()
end
if autoRollReiatsuColorEnabled then
    startAutoRollReiatsuColor()
end
if disableNotificationsEnabled then
    applyNotificationsState()
end
...
if autoRollZanpakutoEnabled then
    startAutoRollZanpakuto()
end
if autoCursedProgressionUpgradeEnabled then
    startAutoCursedProgressionUpgrade()
end
if autoRollCursesEnabled then
    startAutoRollCurses()
end
if autoObeliskEnabled then
    startAutoObelisk()
end
if autoRollDemonArtsEnabled then
    startAutoRollDemonArts()
end
if fpsBoostEnabled then
    applyFPSBoostState()
end
if config.AutoDeleteGachaUnitsToggle then
    startAutoDeleteGacha()
end
if config.AutoChakraProgressionUpgradeToggle then
    startAutoChakraProgressionUpgrade()
end
if autoChakraProgressionUpgradeEnabled then
    startAutoChakraProgressionUpgrade()
end
if autoSoloHunterRankEnabled then
    startAutoSoloHunterRank()
end
if autoReawakeningProgressionUpgradeEnabled then
    startAutoReawakeningProgressionUpgrade()
end
if autoMonarchProgressionUpgradeEnabled then
    startAutoMonarchProgressionUpgrade()
end
if autoSpiritualPressureUpgradeEnabled then
    startAutoSpiritualPressureUpgrade()
end
if config.AutoWaterSpiritProgressionUpgradeToggle then
    startAutoWaterSpiritProgressionUpgrade()
end
if config.AutoWindSpiritProgressionUpgradeToggle then
    startAutoWindSpiritProgressionUpgrade()
end
if config.AutoFireSpiritProgressionUpgradeToggle then
    startAutoFireSpiritProgressionUpgrade()
end
if config.AutoDungeonToggle then
    startAutoDungeon()
end
if config.AutoReiatsuColorToggle then
    startAutoReiatsuColor()
end
if config.AutoSpiritualUpgradeToggle then
    startAutoSpiritualUpgrade()
end
if autoPsychicMayhemEnabled then
...
if config.AutoRollPowerEyesToggle then
    startAutoRollPowerEyes()
end
...
LeftGroupbox:AddToggle(
    "FarmAllToggle",
    {
        ...
        Callback = function(Value)
            ...
            if not Value and getgenv().StopFarmAllThread then
                getgenv().StopFarmAllThread()
            end
            if Value then
                startFarmAll()
...
-- Delay Slider
RollGroupbox:AddSlider(
    "DelayBetweenRollsSlider",
    {
...
DungeonGroupbox:AddButton(
    "Enter Restaurant Raid",
    function()
        local args = {
            [1] = {
                ["Action"] = "_Enter_Dungeon",
    ...
    "Teleport into the Restaurant Raid dungeon."
)
        ...
        if disableNotificationsEnabled then
            ...
            if playerGui then
                ...
                if notifications then
                    if
                        notifications:IsA("ScreenGui") or notifications:IsA("BillboardGui") or
                            notifications:IsA("SurfaceGui")
                     then
                        notifications.Enabled = true
                    elseif notifications:IsA("GuiObject") then
                        notifications.Visible = true

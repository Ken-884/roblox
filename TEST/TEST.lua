...
InfoGroup:AddLabel("Version: 3.1.0")
...
-- Helper to get monster display titles
-- ...existing code...


-- Replace getNearestValidMonster function
-- ...existing code...
...
local function startAutoFarm()
    task.spawn(function()
        ...
        while getgenv().SeisenHubRunning and isAuraEnabled do
            ...
            -- If no current target, or it's dead/gone, or it's no longer in the selected list, pick a new one and teleport
...
local function detectAnyDungeonAndLoopTeleport()
    local dungeonNames = {
        "Dungeon_Easy",
        "Dungeon_Medium",
        "Dungeon_Hard",
    ...
    for _, dungeon in ipairs(dungeonFolder:GetChildren()) do
        for _, baseName in ipairs(dungeonNames) do
            if string.sub(dungeon.Name, 1, #baseName) == baseName then
                foundDungeon = true
                break
            end
        end
    ...
    if foundDungeon then
        ...
        local currentTarget = nil
        local maxDistance = 100 -- Only teleport if monster is within this distance

        while getgenv().SeisenHubRunning and config.AutoDungeonToggle do
            local needTeleport = false
            if not currentTarget
                or not currentTarget:IsDescendantOf(monstersFolder)
            ...
            then
                ...
                local dist = (currentTarget.HumanoidRootPart.Position - myHRP.Position).Magnitude
                if dist > maxDistance then
                    -- Don't teleport if monster is too far away
                    currentTarget = nil
                    needTeleport = false
                end
            end
            ...
            if needTeleport then
                -- Find closest alive monster within maxDistance
                local closestMonster, closestDist = nil, math.huge
                local myPos = myHRP.Position
                for _, monster in ipairs(monstersFolder:GetChildren()) do
                    local hrp = monster:FindFirstChild("HumanoidRootPart")
                ...
                if closestMonster then
                    ...
                    pcall(function()
                        myHRP.CFrame = CFrame.new(currentTarget.HumanoidRootPart.Position + Vector3.new(0, 0, -3))
                    end)
                else
                    currentTarget = nil
                end
            ...
            -- Kill aura: attack current target if valid
...
if config.AutoDungeonToggle then detectAnyDungeonAndLoopTeleport() end
...
-- Set dropdown value after creation to avoid nil errors
if config.MonsterSelector and monsterDropdown and typeof(monsterDropdown.SetValue) == "function" then
    monsterDropdown:SetValue(config.MonsterSelector)
end
...
DungeonGroupbox:AddToggle("AutoDungeonToggle", {
    ...
    Callback = function(Value)
        ...
        saveConfig()
        if Value then
            while config.AutoDungeonToggle and getgenv().SeisenHubRunning do
                detectAnyDungeonAndLoopTeleport()
                task.wait(0.2)
            end
            -- AutoDungeon runs independently now
        end
    end
})
